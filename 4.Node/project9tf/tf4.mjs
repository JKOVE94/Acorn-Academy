...
async function fetchHousingData() {
    const response = 
await fetch('https://raw.githubusercontent.com/selva86/datasets/master/BostonHousing.csv');
...
// 학습률을 명시적으로 설정
    model.compile({loss: 'meanSquaredError', optimizer: tf.train.sgd(0.001)}); // 기본값은 0.01
...
function predictPrice() {
    ...
    if (!isNaN(crimInput) && !isNaN(roomsInput) && !isNaN(lstatInput) && model) {
        ...
        document.getElementById('singlePrediction').textContent = 
`예측된 가격: ${prediction.toFixed(2)}`;    // 예측 결과를 화면에 표시
    } else {
        // 모든 값을 입력하지 않은 경우 경고 메시지 표시
        document.getElementById('singlePrediction').textContent = '모든 값을 입력해주세요!'; 
}
}
